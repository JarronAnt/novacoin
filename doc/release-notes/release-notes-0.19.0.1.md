Bitcoin Core version 0.19.0.1 is now available from:
  ...
  <https://novacoincore.org/bin/novacoin-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/novacoin/novacoin/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `novacoind -help` output for these two options for
...
- The `testnet` field in `novacoin-cli -getinfo` has been renamed to
...
- If a Tor hidden service is being used, Bitcoin Core will be bound to
  the standard port 9191 even if a different port is configured for
...
- #15639 novacoin-wallet tool: Drop libnovacoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in novacoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "novacoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of novacoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from novacoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_novacoin to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_novacoin output (practicalswift)
...
- #15896 feature_filelock, interface_novacoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin Core (luke-jr)
...
- #16277 Suppress output in test_novacoin for expected errors (gertjaap)
...
- #16489 log: harmonize novacoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (novacoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example novacoin.conf (fanquake)
...
- #16080 build/doc: update novacoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in novacoin.conf (fanquake)
...
- #16621 add default novacoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the novacoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, novacoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/novacoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/novacoin/novacoin/).
