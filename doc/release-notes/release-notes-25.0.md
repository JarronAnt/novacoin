...
Bitcoin Core version 25.0 is now available from:
  ...
  <https://novacoincore.org/bin/novacoin-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/novacoin/novacoin/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `novacoin-cli` tool both take advantage of this, so
  for example:

```sh
novacoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
novacoin-cli -named createwallet mywallet load_on_startup=1
```
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching novacoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/novacoin/novacoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
