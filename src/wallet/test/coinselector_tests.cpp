// Copyright (c) 2017-2022 The Novacoin Core developers
...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(result14->GetSelectedValue(), 1 * COIN);  // we should get 1 NOC in 1 coin
        ...
        BOOST_CHECK_EQUAL(result15->GetSelectedValue(), 2 * COIN);  // we should get 2 NOC in 1 coin
...
BOOST_AUTO_TEST_CASE(coin_grinder_tests)
{
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19,8 NOC total --> 60 × 272 WU = 16320 WU
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 NOC total --> 10 × 272 WU = 2720 WU
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            return available_coins;
        });
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            // Expected Result: 4 + 3 + 2 + 1 = 10 NOC at 400 vB
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 10; ++j) {
                /* 10 × 1 NOC + 10 × 2 NOC = 30 NOC. 20 × 272 WU = 5440 WU */
...
BOOST_AUTO_TEST_CASE(check_max_selection_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 NOC and 1515x 0.033 NOC (~100.0 NOC total) unspent outputs
        // Then tries to spend 49.5 NOC
        // The 50.0 NOC output should be selected, because the transaction would otherwise be too large
        ...
        BOOST_CHECK(result);
        // Verify that the 50 NOC UTXO was selected, and result is below max_weight
...

    {
        ...
        // The actor starts with 400x 0.0625 NOC and 2000x 0.025 NOC (75.0 NOC total) unspent outputs
        // Then tries to spend 49.5 NOC
...

    {
        ...
        // The actor starts with 1515x 0.033 NOC (49.995 NOC total) unspent outputs
...
BOOST_AUTO_TEST_CASE(SelectCoins_effective_value_test)
{
    ...
    CoinsResult available_coins;
    {
        ...
        add_coin(available_coins, *dummyWallet, 100000); // 0.001 NOC
    }

    CAmount target{99900}; // 0.000999 NOC
