...
static void SetupNovacoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " novacoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The novacoin-tx tool is used for creating and modifying novacoin transactions.\n\n"
                "novacoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded novacoin transaction, or with \"-create [commands]\" to create a hex-encoded novacoin transaction.\n"
                "\n"
                "Usage: novacoin-tx [options] <hex-tx> [commands]\n"
                "or:    novacoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded novacoin transaction
