// Copyright (c) 2023 The Novacoin Core developers
...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_novacoin", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_novacoin", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_novacoin", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_novacoin", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_Novacoin Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_novacoin.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("novacoin_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("novacoin_sock1_XXXXXX")),
    };
