...
CJDNS peers than Tor or IP ones. You can use `novacoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/novacoin/novacoin/blob/master/doc/tor.md) for
details.
...
To see which CJDNS peers your node is connected to, use `novacoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `novacoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `novacoin-cli help getnodeaddresses` for details.
