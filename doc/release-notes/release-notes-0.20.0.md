...
Bitcoin Core version 0.20.0 is now available from:
  ...
  <https://novacoincore.org/bin/novacoin-core-0.20.0/>
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/novacoin/novacoin/issues/16599),
  [PR #16702](https://github.com/novacoin/novacoin/pull/16702), and the
  `novacoind help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://novacoincore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/novacoin-core/HWI) for
...
- Bitcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.novacoincore.org.  (#17596)
...
- The `novacoin-cli` utility used with the `-getinfo` parameter now
...
- The `novacoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=9191` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `novacoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_novacoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_novacoin` binary (hebasto)
...
- #18702 Fix ASLR for novacoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate novacoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest novacoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_novacoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (novacoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone novacoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken novacoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from novacoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.novacoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to novacoin-wallet (hel-o)
...
- #16115 On novacoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from novacoin-cli -getinfo (malevolent)
