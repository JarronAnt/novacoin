...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<NovacoinUnit>("NovacoinUnit");
#else
    qRegisterMetaType<NovacoinUnit>("NovacoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. novacoin_de.qm (shortcut "de" needs to be defined in novacoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. novacoin_de_DE.qm (shortcut "de_DE" needs to be defined in novacoin.qrc)
...
static const char* qt_argv = "novacoin-qt";

NovacoinApplication::NovacoinApplication()
...
void NovacoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the NovacoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", NovacoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void NovacoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool NovacoinApplication::createOptionsModel(bool resetSettings)
{
...
void NovacoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new NovacoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &NovacoinGUI::quitRequested, this, &NovacoinApplication::requestShutdown);
...
void NovacoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void NovacoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool NovacoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void NovacoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &NovacoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &NovacoinApplication::handleRunawayException);
    connect(this, &NovacoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &NovacoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void NovacoinApplication::parameterSetup()
{
...
void NovacoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void NovacoinApplication::requestInitialize()
{
...
void NovacoinApplication::requestShutdown()
{
...
void NovacoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // novacoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NovacoinGUI::handlePaymentRequest);
            connect(window, &NovacoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void NovacoinApplication::handleRunawayException(const QString &message)
{
...
void NovacoinApplication::handleNonFatalException(const QString& message)
{
...
WId NovacoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool NovacoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", NovacoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(novacoin);
    Q_INIT_RESOURCE(novacoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see novacoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see novacoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse novacoin.conf, determine network, switch to network specific
    ...
    // novacoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
