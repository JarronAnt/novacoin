...
```
novacoind -i2psam=127.0.0.1:7656
```
...
debug log about your I2P configuration and connections. Run `novacoin-cli help
...
attacks](https://en.novacoin.it/wiki/Weaknesses#Sybil_attack). You can use
`novacoin-cli -addrinfo` to see the number of I2P addresses known to your node.
...
To see which I2P peers your node is connected to, use `novacoin-cli -netinfo 4`
or the `getpeerinfo` RPC (e.g. `novacoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `novacoin-cli help getnodeaddresses` for details.
