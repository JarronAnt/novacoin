...
/** Call node.updateRwSetting() with Novacoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Novacoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/novacoin/novacoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because novacoin 22.x will not try to read these.
...
//! Convert enabled/size values to novacoin -prune setting.
...
//! Get pruning enabled value to show in GUI from novacoin -prune setting.
...
//! Get pruning size value to show in GUI from novacoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayNovacoinUnit")) {
        settings.setValue("DisplayNovacoinUnit", QVariant::fromValue(NovacoinUnit::NOC));
    }
    QVariant unit = settings.value("DisplayNovacoinUnit");
    if (unit.canConvert<NovacoinUnit>()) {
        m_display_novacoin_unit = unit.value<NovacoinUnit>();
    } else {
        m_display_novacoin_unit = NovacoinUnit::NOC;
        settings.setValue("DisplayNovacoinUnit", QVariant::fromValue(m_display_novacoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if novacoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_novacoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<NovacoinUnit>() == m_display_novacoin_unit) return;
    m_display_novacoin_unit = new_unit.value<NovacoinUnit>();
    QSettings settings;
    settings.setValue("DisplayNovacoinUnit", QVariant::fromValue(m_display_novacoin_unit));
    Q_EMIT displayUnitChanged(m_display_novacoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/novacoin/novacoin/pull/8273
    ...
    // (https://github.com/novacoin-core/gui/issues/567).
