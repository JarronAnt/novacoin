...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing novacoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='novacoin-cli', help='novacoin-cli command to use')
...
parser.add_argument('novacoin_cli_args', nargs='*', help='Arguments to pass on to novacoin-cli (default: -signet)')

args = parser.parse_args()

if args.novacoin_cli_args == []:
    args.novacoin_cli_args = ['-signet']


def novacoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.novacoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = novacoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = novacoin_cli(['getnewaddress', 'faucet', 'bech32'])
